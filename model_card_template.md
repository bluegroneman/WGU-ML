...
- Purpose: Educational ML pipeline for tabular income classification on the UCI Adult/Census dataset; demonstrates preprocessing, model training, evaluation, and basic serving.
- Input: A single record with the fields listed above.
- Output: Binary prediction string ("<=50K" or ">50K").
- Intended users: Students/practitioners learning ML pipelines.
...
- Source: UCI Adult (Census Income) dataset located at data/census.csv.
...
- Consider probability calibration and threshold tuning if used beyond demonstration; current pipeline returns hard labels.
